...
def get_tasks(brand=None):
    filters_list = [{"property": "진행상태", "select": {"equals": "📋 할일"}}]
    if brand:
        filters_list.append({"property": "브랜드", "select": {"equals": brand}})

    body = {
        "page_size": 100,
        "filter": {"and": filters_list} if len(filters_list) > 1 else filters_list[0],
    ...
    for r in res.json().get("results", []):
        ...
        cat_str = cat.get("name", "") if cat else "기타"
...
def build_briefing(brand=None):
    ...
    label = brand if brand else "전체"
    lines = [f"📋 *{today} {label} 할일*\n"]
...
async def briefing(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    if not is_allowed(update):
        return
    await update.message.reply_text(build_briefing(), parse_mode="Markdown")
...
async def brand_briefing(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(build_briefing(brand), parse_mode="Markdown")
